...
class TestBase(unittest.TestCase):
    ...
    def new_client(self):
        ...
        return self.client
    def new_client2(self):
        assert not hasattr(self, 'client2')
        try:
            self.client2 = client.Client()
        except socket.error:
            self.fail("No se pudo establecer una segunda conexión al server")
        return self.client2
...
class TestHFTPServer(TestBase):
    ...
    def test_multiple_clients(self):
        ...
        def client_task(client, thread_finished):
            client.get_slice(self.output_file, 0, len(test_data))
            thread_finished.set()

        def stop_thread(thread):
            if thread.is_alive():
                thread._stop()  # Cerrar el hilo a la fuerza
        ...
        # Crear y ejecutar los hilos para cada cliente
        thread_finished1 = threading.Event()
        t1 = threading.Thread(target=client_task, args=(c1, thread_finished1))

        thread_finished2 = threading.Event()
        t2 = threading.Thread(target=client_task, args=(c2, thread_finished2))

        t1.start()
        t2.start()

        # Programar la función de detención si se alcanza el timeout
        timer1 = threading.Timer(5, stop_thread, args=(t1,))
        timer2 = threading.Timer(5, stop_thread, args=(t2,))
        timer1.start()
        timer2.start()
        ...
        # Verificar si los hilos se cerraron debido a timeout o finalizaron normalmente
        self.assertTrue(thread_finished1.is_set() or not t1.is_alive(), "El hilo 1 se bloqueó o tardó demasiado")
        self.assertTrue(thread_finished2.is_set() or not t2.is_alive(), "El hilo 2 se bloqueó o tardó demasiado")
        ...
        f = open(self.output_file)
        self.assertEqual(f.read(), test_data, "El contenido del archivo no es el correcto")
        f.close()
